...
class YetAnotherWeatherCard extends HTMLElement {
  ...
  fetchWeatherData() {
    ...
    fetch(pointUrl)
      ...
      .then(function(results) {
        ...
        // Fetch radar data after weather data is loaded
        if (self._config.show_radar) {
          self.fetchRadarData();
        }
      })
  ...
  fetchRadarData() {
    ...
    // Generate timestamps for animation frames (last 50 minutes, 5-minute intervals)
    var now = new Date();
    var frameTimestamps = [];
    ...
    // Generate radar frames (simulated for demo)
    this._radarFrames = [];
    for (var i = 0; i < frameTimestamps.length; i++) {
      this._radarFrames.push({
        timestamp: frameTimestamps[i],
        url: this.generateRadarImageUrl(radarStation, frameTimestamps[i]),
  ...
  generateRadarImageUrl(station, timestamp) {
    // For demo purposes, generate a sample radar image URL
    // In production, this would use actual NWS radar services
    var timeStr = timestamp.toISOString().slice(0, 19).replace(/[:-]/g, '');
    return 'https://via.placeholder.com/600x600/1a1a2e/16537e?text=Radar+' + station + '+' + timeStr.slice(-6);
  }
  ...
  renderRadarSection() {
    ...
    html += '<div class="section-header">Animated Radar' + (station ? ' (' + station + ')' : '') + '</div>';
    ...
    if (!hasFrames) {
      ...
      html += '<img id="radar-image" src="' + currentFrame.url + '" alt="Weather Radar" style="width: 100%; height: 100%; object-fit: cover;" />';
